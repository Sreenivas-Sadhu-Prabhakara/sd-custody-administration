...
| **Asset Type** | Custody Holding |
| **Control Record** | Custody Holding Administrative Plan |
...
| POST | `/v1/custody-holding-administrative-plan/initiate` | Initiate |
| GET | `/v1/custody-holding-administrative-plan` | Retrieve (list) |
| GET | `/v1/custody-holding-administrative-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/custody-holding-administrative-plan/{crId}/update` | Update |
| PUT | `/v1/custody-holding-administrative-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/custody-holding-administrative-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-custody-administration:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-custody-administration ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-custody-administration` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
